...
export default function Competition() {
  ...
  const [competitions, setCompetitions] = useState<EventType[]>([]);
  ...
  useEffect(() => {
    const fetchCompetitions = async () => {
      try {
        ...
        console.error("Gagal mengambil data kompetisi:", error);
      }
    };
    fetchCompetitions();
  }, []);

  const getImageForCompetition = (name: string) => {
    if (name.includes("Poster")) return "https://www.invofest-harkatnegeri.com/assets/competition-card/software_dev.png";
    if (name.includes("UI/UX") || name.includes("UI UX")) return "https://www.invofest-harkatnegeri.com/assets/competition-card/ui_ux.png";
    return "https://www.invofest-harkatnegeri.com/assets/competition-card/web_design.png";
  };
  ...
  return (
    <div style={{ backgroundColor: "#FFFFFF" }}>
        ...
        buttons={[
          {
            ...
            variant: "primary",
            onClick: () => {
              const el = document.getElementById("daftar-kompetisi");
      ...
      <section id="daftar-kompetisi" style={{ padding: "100px 24px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 32 }}>
          {competitions.map((cat, idx) => (
            ...
            >
              <img src={getImageForCompetition(cat.name)} alt={cat.name} style={{ width: "100%", height: "auto", display: "block" }} />
              <div style={{ padding: 28, flexGrow: 1, display: "flex", flexDirection: "column" }}>
                <h3 style={{ fontSize: 24, fontWeight: 800, color: "#1F2937", marginBottom: 16 }}>{cat.name}</h3>
